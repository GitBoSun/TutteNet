...
class ExportTSDFMesh(Exporter):
    ...
    def main(self) -> None:
        ...
        if not self.output_dir.exists():
            self.output_dir.mkdir(parents=True)
        _, pipeline, _, _ = eval_setup(self.load_config, add_deformation=self.add_deformation,
                                       tutte_learning_path=self.tutte_learning_path,
                                       tutte_deform_path=self.tutte_deform_path,
                                       shape_name=self.shape_name, keep_shape=self.keep_shape,
