...
def render_trajectory(
    ...
    """
    #im_out_path = 'images/n3n1'
    #im_out_path = 'images/n11l8_185'

    im_out_path = str(output_dir).replace('tsdf', 'images')
    ...
    with progress:
        for camera_idx in progress.track(range(cameras.size), description=""):
            ...
            rgb_image = cv2.cvtColor(outputs[rgb_output_name].cpu().numpy()*255., cv2.COLOR_BGR2RGB)
            cv2.imwrite( os.path.join(im_out_path, '%d.png'%(camera_idx)), rgb_image)
