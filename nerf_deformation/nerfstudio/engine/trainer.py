...
class Trainer:
            ...
            for step in range(self._start_step, self._start_step + num_iterations):
                ...
                with self.train_lock:
                    with TimeWriter(writer, EventName.ITER_TRAIN_TIME, step=step) as train_t:
                        ...
                        if self.pipeline.model.config.dump_density and step > self._start_step + num_iterations-200:
                            # print('activate dump density')
                            self.pipeline.model.field.dump_density = True 
                            if step > self._start_step + num_iterations-2:
                                self.pipeline.model.field.build_dense_mesh = True
    ...
    def train_iteration(self, step: int) -> TRAIN_INTERATION_OUTPUT:
        ...
        for _ in range(self.gradient_accumulation_steps):
            ...
            if not self.config.add_deformation:
