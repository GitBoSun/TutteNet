...
class CoordinateEncoderChain(nn.Module):
    def __init__(self, mesh, bound_verts, model_cfg, runtime_cfg):
        ...
        self.append_bn = model_cfg.get("APPEND_BN", True)
        ...
        if  self.use_positional_encoding:
            ...
            encoding_channel = self.encoding_length * 4
        else:
            encoding_channel = 2 
        ...
        # self.fc_w = self.make_fc_layers([self.fc1_dim, self.fc1_dim, self.fc1_dim*2, self.fc1_dim*2, self.fc1_dim,], input_channels, self.num_layers*self.num_innder_layer,)
        # self.fc_a = self.make_fc_layers([self.fc1_dim, self.fc1_dim, self.fc1_dim*2, self.fc1_dim*2, self.fc1_dim ], input_channels, self.num_layers*self.num_innder_layer,) 
    
        self.fc1 = self.make_fc_layers([self.fc1_dim, self.fc1_dim*2, self.fc1_dim*2], input_channels, self.fc1_dim,)
        fc_layers = []
        self.latent_dim = self.fc1_dim

        for i in range(self.num_layers):
            layer1_w = self.make_fc_layers([self.latent_dim, ], self.latent_dim+encoding_channel, self.num_innder_layer,)
            layer1_a = self.make_fc_layers([self.latent_dim,], self.latent_dim+encoding_channel, self.num_innder_layer,)  
            # layer1_w = self.make_fc_layers([self.latent_dim, self.latent_dim//2], self.latent_dim+encoding_channel, self.num_innder_layer,)
            # layer1_a = self.make_fc_layers([self.latent_dim, self.latent_dim//2], self.latent_dim+encoding_channel, self.num_innder_layer,)  
            if i<self.num_layers-1:
                layer2 = self.make_fc_layers([self.latent_dim,], self.latent_dim, self.latent_dim, append_bn=self.append_bn) 
                # layer2 = self.make_fc_layers([self.latent_dim, self.latent_dim], self.latent_dim, self.latent_dim, append_bn=self.append_bn) 
            else:
                layer2 = None 
            fc_layers.append(nn.ModuleList([layer1_w, layer1_a, layer2]))
    ...
    def positional_encoding(self,input_xy, L=2):
        # [N, 2]
        x = input_xy[:, 0]
        y = input_xy[:, 1]
    ...
    def forward(self, batch_dict):
        ...
        for i in range(self.num_layers):
            ...
            w_input = torch.cat((las_latent_code.unsqueeze(1).repeat(1, self.num_edges,1),\
                self.edge_centers.unsqueeze(0).repeat(batch_size, 1,1)), dim=2) # [B,M,74]
            w_input = w_input.view(batch_size*self.num_edges, -1)

            a_input = torch.cat((las_latent_code.unsqueeze(1).repeat(1, self.num_bound_verts,1),\
                self.bound_verts.unsqueeze(0).repeat(batch_size, 1,1)), dim=2) # [B,M,74]
            a_input = a_input.view(batch_size*self.num_bound_verts, -1)
        
            w_pred = layers_i[0](w_input)
            a_pred = layers_i[1](a_input)
            w_pred = w_pred.view(batch_size, self.num_edges, self.num_innder_layer)
            a_pred = a_pred.view(batch_size, self.num_bound_verts, self.num_innder_layer) 
            
            var_pred = torch.cat((w_pred, a_pred), dim=1)
            
            if i<self.num_layers-1:
                las_latent_code = layers_i[2](las_latent_code) 
