...
class LatentEncoder(nn.Module):
    def __init__(self, model_cfg, runtime_cfg, mesh=None,bound_verts=None ):
        super().__init__()
        ...
        self.share_mlp = model_cfg.get("SHARE_MLP", False) 
        ...
        self.fc1 = self.make_fc_layers([self.fc1_dim, self.fc1_dim*2, self.fc1_dim*4], input_channels, self.fc1_dim * 4,)
        fc_layers = []
        input_channel = self.fc1_dim*4
        self.latent_dim = input_channel 
        
        if self.share_mlp:
            # self.layer0 = self.make_fc_layers([self.latent_dim,self.latent_dim ], self.latent_dim, self.latent_dim, )  
            self.layer1 = self.make_fc_layers([self.latent_dim, self.latent_dim*2, self.latent_dim*2,self.latent_dim*4], self.latent_dim, num_var,)  
            self.layer2 = self.make_fc_layers([self.latent_dim, self.latent_dim, self.latent_dim], self.latent_dim, self.latent_dim, append_bn=self.append_bn)  
        else:
            for i in range(self.num_layers):
                # layer1 = self.make_fc_layers([self.latent_dim*2], self.latent_dim, num_var+ self.latent_dim) 
                # fc_layers.append(layer1)
                layer1 = self.make_fc_layers([self.latent_dim*2], self.latent_dim, num_var,) 
