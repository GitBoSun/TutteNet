...
class TutteTriplane(nn.Module):
    ...
    def forward(self, input_points, var_pred):
        ...
        # cur_points2 = new_points1[:,:,1:3].clone()
        # pred_points2, new_vertices2 = self.tutte_layer2(cur_points2, var_pred[:,:,1])
        # new_points2 = torch.cat((new_points1[:,:,0].unsqueeze(2), pred_points2), dim=2)
        
        # cur_points3 = torch.cat((new_points2[:,:,0].unsqueeze(2), new_points2[:,:,2].unsqueeze(2)), dim=2)
        # pred_points3, new_vertices3 = self.tutte_layer3(cur_points3, var_pred[:,:,2])
        # new_points3 = torch.cat((pred_points3[:,:,0].unsqueeze(2), new_points2[:,:,1].unsqueeze(2), pred_points3[:,:,1].unsqueeze(2)), dim=2) 
...
class TutteHead3D(nn.Module):
    ...
    def __init__(self, mesh, bound_verts, runtime_cfg, model_cfg, **kwargs):
        ...
        self.num_points1 = runtime_cfg["num_points1"]
        self.num_points2 = runtime_cfg["num_points2"]
    ...
    def get_regress_loss(self, tb_dict=None, prefix=None):
        ...
        if self.area_loss_weight:
            e_param = self.forward_ret_dict['pose_param']
            area_weights = e_param[:, 0] * e_param[:, 1] * e_param[:, 3]
    ...
    def get_loss(self, tb_dict=None, prefix=None):
        ...
        if self.add_layer_distortion_loss:    
            for i in range(self.num_layers):
                r_loss += self.get_distortion_loss(self.forward_ret_dict['distortion%d1'%(i)],dim=2)/3
                r_loss += self.get_distortion_loss(self.forward_ret_dict['distortion%d2'%(i)],dim=2)/3
                r_loss += self.get_distortion_loss(self.forward_ret_dict['distortion%d3'%(i)],dim=2)/3
                r_loss = r_loss/self.num_layers
            point_loss += self.r_weight * r_loss 
                
        print("lossssss", point_loss.item(), d_loss.item(),r_loss.item())
        
        return point_loss, tb_dict

    def forward(self, batch_dict):
        batch_size =  batch_dict['batch_size']
        if 'temp_vert' in batch_dict.keys(): 
            temp_points = batch_dict['temp_vert'].repeat(batch_size, 1,1)
            shape_points = batch_dict['shape_points']
            
            fps_ids = batch_dict['fps_ids']
            input_points = temp_points[:, fps_ids, :]
            target_points = shape_points[:, fps_ids, :]
            
        else:
            theta = torch.pi * torch.arange(self.num_points1).float().cuda()/self.num_points1
            phi = 2 * torch.pi * torch.arange(self.num_points2).float().cuda()/self.num_points2
            theta = theta.unsqueeze(1)
            phi = phi.unsqueeze(0)
            x = torch.sin(theta) * torch.cos(phi)
            y = torch.sin(theta) * torch.sin(phi)
            z = torch.cos(theta)
            z = z.repeat(1, self.num_points2)
            input_points = self.input_radius * torch.concat((x.unsqueeze(-1), y.unsqueeze(-1), z.unsqueeze(-1)), dim=-1)
            input_points = input_points.view(-1, 3)
        ...
        if 'temp_vert' in batch_dict.keys(): 
            ret_dict.update({'temp_points': temp_points, 
        ...
        for i in range(self.num_layers):
            ...
            var_pred = var_pred.view(batch_size, -1, 3)
            pred_points, mid_dict = self.tutte_layers[i](pred_points, var_pred)
            batch_dict.update(mid_dict)
            if 'l%d_distortion'%(i) in mid_dict.keys():
                distortion = mid_dict['l%d_distortion'%(i)]
                ...
                ret_dict.update({'distortion%d1'%(i): mid_dict['l%d_distortion1'%(i)], 
                                 'distortion%d2'%(i): mid_dict['l%d_distortion2'%(i)],
                                 'distortion%d3'%(i): mid_dict['l%d_distortion3'%(i)]
                                 })
            ret_dict.update({'var_pred%d'%(i): var_pred})
        
        total_distortion = total_distortion.view(batch_size, self.num_points1* self.num_points2, 3,3)
        ...
        ids = range(self.num_points1*self.num_points2)
        ids1 = range(batch_size)
        jacob = jacob[:, ids, :, :, ids, :][:, ids1, :, ids1, :]
        for i in range(batch_size):
            for j in range(self.num_points1*self.num_points2):
