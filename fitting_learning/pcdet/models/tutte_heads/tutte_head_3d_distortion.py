...
class TutteLayer(nn.Module):
    ...
    def forward(self, input_points, var_pred, inverse=False, depth=None):
        ...
        # N_points = input_points.shape[1]
        # input_nodes = self.find_simplex(input_points)
        # input_nodes =  input_nodes.view(batch_size, N_points, 3).int().cpu().numpy()
    ...
    def find_simplex(self, points,):
        ...
        N_points = points.shape[1]
        points = points.reshape(batch_size*N_points, 2)

        points = points / 2 + 0.5 
        points = torch.clamp(points, 0.05, 0.95) 
        ...
        out_triangle_verts = torch.zeros((n_points, 3)).long().cuda()
        ...
        # print('#####', self.vertices.shape, center_ids.max(), center_ids.min())
        center_ids = torch.clamp(center_ids, 0, self.vertices.shape[0])
        vertices  = self.vertices / 2 + 0.5 
        center_verts = vertices[center_ids]
        
        difference = points - center_verts 
        tangent = difference[:,1] /(difference[:,0] + 1e-4)
    ...
    def find_simplex_2(self, points,):
        N = 11
        batch_size = points.shape[0]
        N_points = points.shape[1]
        points = points.reshape(batch_size*N_points, 2)
        ...
        ### create large tensor 
        out_triangle_verts = torch.zeros((n_points, 3)).long().cuda()
        # out_triangle_verts = self.out_triangle_verts.clone().repeat(batch_size, 1) ##### 
        # out_triangle_verts = self.out_triangle_verts.view(batch_size*N_points, 3)
        top_left_ids = (x_ids * N + y_ids).long()  
        top_right_ids = (x_ids * N + y_ids  + 1 ).long()  
        bottom_left_ids = ((x_ids+1) * N + y_ids).long()  
        bottom_right_ids = ((x_ids+1) * N + y_ids + 1 ).long()  
        ...
        print('##difference', difference.shape, difference.max(0)[0], difference.min(0)[0])
        print('##tagent', tangent.shape, tangent.max(), tangent.min())

        mask1 = torch.nonzero((difference[:,0]>=0) & (tangent <=1) & (tangent >-1), as_tuple=True)[0]
        mask2 = torch.nonzero((difference[:,1]>=0) & ((tangent >1) | (tangent <=-1)), as_tuple=True)[0]
        mask3 = torch.nonzero((difference[:,0]<0) & (tangent <=1) & (tangent >-1), as_tuple=True)[0]
        mask4 = torch.nonzero((difference[:,1]<0) & ((tangent >1) | (tangent <=-1)), as_tuple=True)[0]
        print('##mask', mask1.max(), mask1.min())
        # #### no torch nonzero 
        # mask1 = (difference[:,0]>=0) & (tangent <=1) & (tangent >-1)
        # mask2 = (difference[:,1]>=0) & ((tangent >1) | (tangent <=-1))
        # mask3 = (difference[:,0]<0) & (tangent <=1) & (tangent >-1)
        # mask4 = (difference[:,1]<0) & ((tangent >1) | (tangent <=-1))
...
class TuttePredictingNormal(nn.Module):
    ...
    def forward(self, input_points, var_pred, normal_pred):
        ...
        for i in range(self.num_inner_layer):
            ...
            # print('pred one layer normal time', time.time() - t0)
            new_points_before_rot = torch.cat((pred_points, rot_points[:,:,2].unsqueeze(2)), dim=2) 
            new_points = torch.matmul(self.normal_matrices[:, i].transpose(1,2), \
                new_points_before_rot.transpose(1,2)).transpose(1,2) # [B, N, 3]
            ...
            return_dict[prefix+'pred_points_%d'%(i)] = new_points 
            return_dict[prefix+'pred_points_before_%d'%(i)] = new_points_before_rot
            return_dict[prefix+'new_vertices_%d'%(i)] = new_vertices
            return_dict[prefix+'pred_2d_points_%d'%(i)] = pred_points
            return_dict[prefix+'distortion%d'%(i+1)] = distortions
...
class TutteHead3D_Distortion(nn.Module):
    ...
    def get_regress_loss(self, tb_dict=None, prefix=None):
        ...
        pred_points = pred_points[:, :target_points.shape[1]] #####
    ...
    def get_jacobian_loss(self,):
        ...
        if target_points.shape[1] != pred_points.shape[1]:
        ...
        loss_i = torch.square(pred_J - target_J).mean(-1).mean(-1)
        return loss, loss_i 
    ...
    def get_middle_loss(self):
        ...
        loss = torch.square(target_points - pred_points).mean() /  torch.square(target_points).mean()
        return loss  
    ...
    def get_loss(self, tb_dict=None, prefix=None):
        ...
        if self.add_jacobian_loss:
            j_loss, j_loss_i = self.get_jacobian_loss()
    ...
    def forward(self, batch_dict, inverse=False):
        ...
        ret_dict = {
            ...
            'source_face': batch_dict['source_face'][0], 
        }  
        ...
        for i in range(self.num_layers):
            ...
            # total_points = torch.zeros(pred_points.shape).unsqueeze(0).repeat(24,1,1,1).cuda()
            # total_points_rot = torch.zeros(pred_points.shape).unsqueeze(0).repeat(24,1,1,1).cuda()
            # for ii in range(24):
            #     # points_i = mid_dict['l0_pred_points_%d'%(ii)]
            #     # points_before_i = mid_dict['l0_pred_points_before_%d'%(ii)]
            #     total_points[ii] = mid_dict['l0_pred_points_%d'%(ii)]
            #     total_points_rot[ii] = mid_dict['l0_pred_points_before_%d'%(ii)]
        ...
        ret_dict.update({
            ...
            # 'total_points_rot': total_points_rot,
        })
        
        self.forward_ret_dict = ret_dict
        j_loss, j_loss_i = self.get_jacobian_loss()
        ret_dict.update({'j_loss_i': j_loss_i}) 
