...
mesh_path = '/home/bosun/projects/PCPerception/meshes/amass_pose'
if not os.path.exists(mesh_path):
    os.makedirs(mesh_path)

class SMPLAMASSDataset(DatasetTemplate):
    def __init__(self, dataset_cfg, training=True, root_path=None, logger=None):
        ...
        self.smpl_model = load_model('/home/bosun/projects/SMPL_python_v.1.1.0/smpl/models/basicmodel_f_lbs_10_207_0_v1.1.0.pkl')
        ...
        self.use_all  = self.dataset_cfg.get("USE_ALL", False) 
        ...
        if self.sample_gaussian:
            ...
            self.amass_mean = amass_data.mean(0)
            self.amass_std = np.std(amass_data, axis=0)
    ...
    def __getitem__(self, index):
        ...
        if (not self.sample_gaussian) or (self.split=='val' and self.amass_val) or \
                (self.split=='val' and self.set_val):
            tmp_pose = self.data[index]
            self.smpl_model.pose[:] = tmp_pose
            vert2 = self.smpl_model.r 
        else: 
            tmp_pose = np.random.normal(self.amass_mean, self.amass_std* self.std_factor)
            self.smpl_model.pose[:] = tmp_pose
            vert2 = self.smpl_model.r 
        ...
        mesh = Mesh(v=vert2, f=self.temp_face)
        mesh.write_obj(os.path.join(mesh_path, '%d.obj'%(index)))
        print('saved')

        point_wise_dict = dict(
            pose_param=tmp_pose, 
            # pose_param=self.poses[index],
            shape_points=vert2,
        )
