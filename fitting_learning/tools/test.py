...
def parse_config():
    ...
    parser.add_argument('--ckpt_2', type=str, default=None, help='checkpoint to start from')
    parser.add_argument('--vis_prefix', type=str, default="", help='vis_prefix')
...
def eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=False, rank=0, vis_prefix=""):
    # load checkpoint
    #model.load_params_from_file(filename=args.ckpt, logger=logger, to_cpu=dist_test, ema=args.ema)
    model.load_params_from_file(filename=args.ckpt, logger=logger, to_cpu=dist_test)
    if args.ckpt_2 is not None:
        model.load_params_from_file(filename=args.ckpt_2, logger=logger, to_cpu=dist_test)
    model.cuda()
    ...
    eval_utils.eval_one_epoch(
        ...
        rank=rank, vis_prefix=vis_prefix,
    )
    ...
    with torch.no_grad():
        if args.eval_all:
            ...
            eval_single_ckpt(model, test_loader, args, eval_output_dir, logger, epoch_id, dist_test=dist_test, \
                    rank=cfg.LOCAL_RANK, vis_prefix=args.vis_prefix)
